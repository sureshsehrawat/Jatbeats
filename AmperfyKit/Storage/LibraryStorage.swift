...
public class LibraryStorage: PlayableFileCachable {
  ...
  public func getAllGenres() -> [Genre] {
    let fetchRequest = NSFetchRequest<GenreMO>(entityName: Genre.typeName)
    do {
      let fetchedGenres = try context.fetch(fetchRequest)
      return fetchedGenres.map { Genre(managedObject: $0) }
    } catch {
      os_log("Error fetching all genres: %s", log: log, type: .error, error.localizedDescription)
      return []
    }
  }
}
