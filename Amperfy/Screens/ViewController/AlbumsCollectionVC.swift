...
class AlbumsCollectionVC: SingleSnapshotFetchedResultsCollectionViewController<AlbumMO> {
  ...
  // Cache for cell size calculations
  private var cellSizeCache: [CGFloat: CGSize] = [:]
  
  // Throttling for layout invalidation
  private var lastInvalidationTime: CFTimeInterval = 0
  private let invalidationThreshold: CFTimeInterval = 0.3 // seconds
  ...
  /// Resets the size cache and invalidates the collection view layout
  func resetSizeCacheAndInvalidateLayout() {
  ...
  override func viewDidLayoutSubviews() {
    ...
    // but throttle invalidations to prevent excessive layout recalculations
    let currentSize = view.bounds.size
    let currentTime = CACurrentMediaTime()
    
    if currentSize != previousSize && 
       (currentTime - lastInvalidationTime) > invalidationThreshold {
      previousSize = currentSize
      lastInvalidationTime = currentTime
      
      // Clear size cache when layout changes
      cellSizeCache.removeAll()
      
      // Use performBatchUpdates to ensure smooth animation
...
extension AlbumsCollectionVC: UICollectionViewDelegateFlowLayout {
  func collectionView(
    ...
    // Use cached sizes to avoid recalculating for every cell during scrolling
    let availableWidth = collectionView.bounds.size.width
    if let cachedSize = cellSizeCache[availableWidth] {
      return cachedSize
    }
    ...
    let availableContentWidth = availableWidth - inset.left - inset.right
    
    // Apple Music style: 2 columns on phone, 3-4 on iPad
    let count: CGFloat
    if collectionView.traitCollection.userInterfaceIdiom == .phone {
      count = CGFloat(appDelegate.storage.settings.albumsGridSizeSetting) // Use the setting from user preferences
    } else {
      count = CGFloat(appDelegate.storage.settings.albumsGridSizeSetting)
    }
    
    let artworkWidth = (availableContentWidth - (spaceBetweenCells * (count - 1))) / count
    // Apple Music style height: artwork + title + subtitle - reduced to be more compact
    let size = CGSize(width: artworkWidth, height: artworkWidth + 42)
    
    // Cache the calculated size
    cellSizeCache[availableWidth] = size
  ...
  func collectionView(
    ...
    -> CGFloat {
    // Apple Music uses more space between rows
    return 24.0
  }
  ...
  func collectionView(
    ...
    -> CGFloat {
    // Apple Music uses less space between items in the same row
