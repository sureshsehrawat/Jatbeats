...
class SearchVC: BasicTableViewController {
  ...
  override func configureSearchController(
    ...
    #else
      // Create a UIBarButtonItem with the profile button if it exists
      if let profileButton = profileImageButton {
        let profileButtonItem = UIBarButtonItem(customView: profileButton)
        
        // Add both buttons to the navigation bar
        navigationItem.rightBarButtonItems = [optionsButton, profileButtonItem]
      } else {
...
@available(iOS 14, *)
extension SearchVC: PHPickerViewControllerDelegate {
