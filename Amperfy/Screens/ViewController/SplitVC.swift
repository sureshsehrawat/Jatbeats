...
class SplitVC: UISplitViewController {
  ...
  private func displayNotificationAuthorization() {
    Task { @MainActor in
      ...
      popupVC.setContent(
        topic: "Notifications",
        detailMessage: "Jatbeats can inform you about the latest podcast episodes. If you want to, please authorize Jatbeats to send you notifications.",
        customIcon: .bell,
  ...
  public func visualizePopupPlayer(
    ...
    func togglePopupPlayer() {
      if popupBarContainerVC.popupPresentationState == .open,
         popupBarContainerVC.popupContent is PopupPlayerVC,
         direction != .open {
    ...
    visualizePopupPlayer(direction: .close, animated: true) {
      if self.isCompact {
        if let tabBar = self.viewController(for: .compact) as? TabBarVC,
           let hostingTabViewControllers = tabBar.viewControllers,
           hostingTabViewControllers.count >= 3 {
          tabBar.selectedIndex = 0
          Task {
            try await Task.sleep(nanoseconds: 500_000_000)
            if let libraryNavVC = hostingTabViewControllers[0] as? UINavigationController,
               let searchTabVC = libraryNavVC.visibleViewController as? SearchVC {
  ...
  func displayNewTab() {
    visualizePopupPlayer(direction: .close, animated: true) {
      if self.isCompact {
        ...
        let newVC = TabNavigatorItem.new.controller
        self.setViewController(self.embeddInNavigation(vc: newVC), for: .secondary)
      }
    }
  }
}
