...
class LargeCurrentlyPlayingPlayerView: UIView {
  ...
  func prepare(toWorkOnRootView: PopupPlayerVC?) {
    ...
    // Configure artwork with Apple Music style rounded corners
    configureArtworkStyle()
  ...
  private func configureArtworkStyle() {
    // Apply Apple Music style corner radius
    artworkImage.layer.cornerRadius = CornerRadius.appleMusic.asCGFloat
    artworkImage.layer.masksToBounds = true
    
    // Ensure the content scale aspect properly (Apple Music uses aspectFill)
